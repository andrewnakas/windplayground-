...
class Trainer:
    ...
    def save(self, name: str, step: int, val_loss: float) -> None:
        torch.save(
            {
    ...
    def train(self, benchmark_steps: int = 0) -> dict:
        cfg = self.cfg.train
            ...
            for x, y in loader:
                ...
                if step % cfg.val_every == 0 or done or step >= max_steps:
                    ...
                    if val_loss < best_val:
                        best_val = val_loss
                        self.save("best.pt", step, val_loss)
                if step >= max_steps:
                    done = True
                if done:
                    break
        self.save("last.pt", step, best_val)
        log_f.close()
